...
Table: Measured data for velocity-area method of calculating
streamfllow

+-----------+-------+-----------+-----------+-----------+-----------+
| **Site**  | **    | **Section | **Depth   | **Width   | 
|           | Total | (m)**     | (m)**     | (m)**     | **Velocity|
|           | Width |           |           |           | (m/s)**   |
|           | (m)** |           |           |           |           |
+-----------+-------+-----------+-----------+-----------+-----------+
| Site 01   | 3.8   | 0-0.5     | 0.29      | 0.75      | 0.044     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 0.5-1     | 0.34      | 0.5       | 0.215     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 1-1.5     | 0.26      | 0.5       | 0.227     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 1.5-2     | 0.3       | 0.5       | 0.247     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 2-2.5     | 0.25      | 0.5       | 0.232     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 2.5-3     | 0.2       | 0.5       | 0.091     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 3-3.8     | 0.11      | 0.55      | 0.06      |
+-----------+-------+-----------+-----------+-----------+-----------+
| Site 02   | 3     | 0-0.5     | 0.33      | 0.75      | 0.445     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 0.5-1     | 0.33      | 0.5       | 0.399     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 1-1.5     | 0.35      | 0.5       | 0.277     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 1.5-2     | 0.29      | 0.5       | 0.388     |
+-----------+-------+-----------+-----------+-----------+-----------+
|           |       | 2-3       | 0.23      | 0.75      | 0.424     |
+-----------+-------+-----------+-----------+-----------+-----------+
...
Manning's formula for estimating runoff (in m^3^s^-1^) from basic
...
-   The bottom slope ($S$) will be estimated by using a differential GPS
    (DGPS). Conduct this by averaging the slope (m/m) for two reaches,
    one upstream and one downstream of the measurement point
-   Estimate the 'wetted perimeter' ($P$) of the channel and cross
    sectional area ($A$)
-   The roughness coefficient ($n$) should be assumed from the
    literature, based on your observations and using a photograph of the
    channel. Then use the table below:
...
**Activity:** Look at the demonstration models in the lab of the two
example river/floodplain cross-sections. Assuming these are a scale
model of a real-world river channel, compute the discharge.

-   Sketch and use the scale to get the channel dimensions
-   Estimate the "wetted perimeter"
-   Compute a slope parameter, $S$, assume for every 1km of stream
    length, the stream bed drops by 1m
-   Estimate the cross-sectional area; the area "normal" (perpendicular)
    to the flow velocity
-   Estimate a roughness parameter, Manning's $n$, and solve Equation
    \@ref(eq:exercise2-2)
...
Next, use the graph below (or Excel) to plot a graph of discharge (on the y-axis)
versus recurrence interval (on the x-axis). Note that the x-axis must be a
logarithmic scale. If drawig by hand you should try to estimate as best you can
where the data point will fall between the lines on the graph. Once you
have plotted the points use a ruler to draw the best fit straight line
through the data points (lay a ruler on the graph and try to draw a line
that most closely approximates all of the data points). Do not draw
lines that connect individual data points. If you are plotting in Excel you 
will fit a linear trendline.
