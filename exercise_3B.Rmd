...
Where $\bar{v}$ is the average linear velocity of the water flowing through the porous media. Now that we know these simple equations let's look at some experimental data.
...
**Step 2.** Measure the volume of water that comes out of your tube over a set time and record your data in Table \@ref(tab:E3Btable1).
...
```
```{r E3Btable1, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
options(kableExtra.html.bsTable = F, knitr.kable.NA = '')
table1 <- read.csv("tables/exercise_3B/table1.csv", check.names=FALSE)
...
**Step 1.** Annotate the photo of this model (Figure \@ref(fig:E3Bimage2)) to show the following;
...
```{r E3Bimage2, echo=FALSE, fig.cap="Physical model of a layered groundwater system.", out.width = '80%'}
knitr::include_graphics("images/exercise_3B/picture2.png")
```

</center>

**Step 2.** Watch [this video](https://echo360.org.au/media/beef71b3-fdca-4a62-9a58-4ad9f0b7b58a/public) to see how water moves through the model. Now let's inject a dye to see how fast the water is moving and in which direction.
