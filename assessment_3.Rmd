...
The time-lag of one day ($t-1$) and two days ($t-2$), is a way to simply account for the time it takes for water in sub-catchment 1 to reach to overall catchment outlet, a process we call streamflow routing.
...
2)  Confirm what the indicies are refering to in the above equations ($t$, $i$).
3)  Build the above model in Excel, to predict the individual sub-catchment flows and river basin total flow. Assume basic information as outlined in Table \@ref(tab:tableA3A1) below and the supplied rainfall data for the 3 sub-catchments. Input a gussed value for $c$ within the range indicated for now.
...
### Calibrating the model {#A3A_7 .unnumbered}
...
5)  Plotting the time-series of daily flows is“crowded” and it’s hard to see differences. In this case we therefore do what is called a “cumulative mass-curve”. For this we cumulate the daily flow and plot over time – so it keeps increasing. This is the cumulative mass of water (usally presented in m3) delivered by the catchment. Create a "stacked area plot" of the cumulative flow from each sub-catchment.


### Land-use change scenario {#A3A_9 .unnumbered}
...
## B. Lake system model {#A3A_3 .unnumbered}
...
### Assessing the model {#A3A_5 .unnumbered}
...
## C. Climate change impact assessment {#A3A_3 .unnumbered}
...
- How managers could use this to help reduce nutrient export from a catchment and adapt to a changing climate
...
### Optional: Add phosphorus concentrations to the model{#A3A_8 .unnumbered}
