...
The following LIDAR image shows the surface topography, with the
...
the ground and lined with PVC that has slots at the bottom to let the
...
interval? This is the actual elevation of your measurement; it's
important that you know this so that you can compare your measurement
with existing data. Once you get to the well, the first thing to do is
...
well (standing water). As a rule of thumb you need to remove 3-times
this volume of standing water from the well before you sample for
...
hydrologic pathways (surface and sub-surface) that determine
distribution of water and salt distribution in this floodplain system.
You will use this information to develop a hydrological conceptual model
of this site. Your will assessment report should describe the context of
the investigation and summarise your findings and observations as
suitable for use by the City of Swan Environmental Management team.
...
-   Identify the surface water flow regime - where and when does surface
    water pond or flow
-   Summarize the soils found at the site and their key properties
-   Articulate the key features of the groundwater system - water level
    trends, groundwater flow, surface water - groundwater interaction
-   Assess the distribution of salinity in soils, groundwater wells
...
You are provided with a series of .csv files that summarise the
timeseries data collected by UWA and DWER (via WIR). This includes:

-   Baro: This was a UWA logger measuring atmospheric pressure, in kPa.
    We measure this as the water levels in the piezometers may vary due
    to atmospheric pressure fluctatations. For your report you do not
    need to present this, just bear in mind it was used to correct the
    piezometer water level data.

-   Level: This includes the UWA piezometer level data (converted into
    mAHD) for HW1, HW2, and HW3. Bore 7 is also included which is
    denoted as Jeremy on the map. Column G River Corrected is the water
    level of Helena River under the Bridge. So you can think of this as
    the water level at the river-end of the piezometer transect.

-   Salinity: This has the UWA salinity measurements from inside the
    Piezometers (and occasional river samples). These data are in mS/cm.

-   MSB and PoisonLease: This includes the WIR data from two measurement
    points; "MSB" (in the main Swan River, to the west of our site) and
    "Poison Lease" (upstream in Helena River to the east of our site).
    Note that Column E has the discharge (Q) at this site, based on the
    above rating curve.

-   MSB levels: This has the same MSB data as above, but includes the
    daily min and mix, showing the tidal amplitude of the Swan River
    over time.

-   Rain: This has daily rainfall data from the nearby DPIRD South-Perth
    weather station.

-   SwanRiver Vitox Salinity: This has data from the WIR website for the
    "Vitox" location which is just to the west of our site (on the other
    side of Kings Meadow Oval). This data indicates the river/estuary
    salinity and would be similar to the salinity of the surface water
...
-   Remember, when working with groundwater data you need to be careful
    ...
    to groundwater levels the Australian Height Datum (AHD). If you have
...
-   How does salinity vary across the surface water sites? How does this
    relate to sources of water (Helen River, Stormwater from nearby
    suburbs, Swan estuary, Rain)

-   How does salinity vary in the groundwater based on distance from the
    river?
...
-   Are is the unsaturated zone (vadose zone) adequate for the tree
    roots? Ie is the water table depth deep enough to contain non-salty
...
### Optional: Helena River Floodplain Self-paced site inspection {#E4_15 .unnumbered}
