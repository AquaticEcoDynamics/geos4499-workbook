# Exercise 6 - Simple lake model {#E6_1 .unnumbered}

## Introduction {#E6_2 .unnumbered}
...
## Defining the system {#E6_3 .unnumbered}

In this first part of the assignment you will use a simple "bucket" storage model to simulate changes in lake water level through time. The depth of the lake increases due to rainfall and inflows and decreases due to evaporation and seepage (Figure \@ref(fig:E6image1)). There is also a maximum level to which the lake can be filled, $h_{\text{max}}$ (m), above which water will overflow from the lake, such that $0\le h\le h_{\text{max}}$. This simple system is described by the equation;
...
## Transient simulation {#E6_5 .unnumbered}
...
## Rates of change {#E6_6 .unnumbered}

Now let's look at the rate of change in lake level under each of these modelling scenarios. **Plot** $\frac{dh}{dt}$ vs time for each model (hint: remember that this is just the change in water level over time). How does this plot of $\frac{dh}{dt}$ vs time relate to Figure \@ref(fig:E6image2)? How long does it take for each model to reach "steady state"? How would the time it takes to reach steady state change if we used a different initial water level in our simulations?

Now have a go at making your own rainfall boundary condition. You may choose to base it on a probability distribution, or a have it randomly varying through time. **Plot both lake water level and** $\frac{dh}{dt}$ vs time for your chosen rainfall distribution. How does this change your modelling results?
